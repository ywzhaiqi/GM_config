 ...
 if(document.evaluate("//iframe[@id='GM_config']",document,null,9,null).singleNodeValue) return;
	...
	GM_config.frame.addEventListener('load', function() {
		...
		frameBody.appendChild(obj.create('div', {id:'buttons_holder', kids:new Array(
			obj.create('button',{id:'saveBtn',textContent:'确定',title:'部分选项需要刷新页面才能生效',className:'saveclose_buttons',onclick:function(){GM_config.close(true)}}),
			obj.create('button',{id:'cancelBtn', textContent:'取消',title:'取消本次设置，所有选项还原',className:'saveclose_buttons',onclick:function(){GM_config.close(false)}}),
			obj.create('div', {className:'reset_holder block', kids:new Array(
				obj.create('a',{id:'resetLink',textContent:'恢复默认设置',href:'#',title:'恢复所有设置的内容为默认值',className:'reset',onclick:obj.reset})
		)}))}));
		...
		switch(field.type) {
				case 'textarea':
					elem = create(isKid ? "span" : "div", {title:field.title||'', kids:new Array(
						create('span', {innerHTML:label, className:'field_label'}),
				...
				case 'radio':
					...
					for (var j = 0,len = Options.length; j<len; j++) {
						boxes.push(create('span', {innerHTML:Options[j]}));
				...
				case 'select':
					var options = [];
					if(typeof Options === "object" && typeof Options.push !== "function") for(var j in Options) options.push(create('option',{innerHTML:Options[j],value:j,selected:(j==value)}));
						else options.push(create("option", {innerHTML:"Error - \"options\" needs to be a JSON object.", value:"error", selected:"selected"}));
					elem = create(isKid ? "span" : "div", {title:field.title||'', kids:new Array(
						create('span', {innerHTML:label, className:'field_label'}),
				...
				case 'checkbox':
					elem = create(isKid ? "span" : "div", {title:field.title||'', kids:new Array(
						create('label', {innerHTML:label, className:'field_label', "for":'field_'+i}),
 ...
 * TODO：从 GM_config Extender 修改而来，还有 bug，有待测试和修正
 */
var sectionTabs = 0; // holds the number of tabs we have
GM_config.toggleSection = function(e) { // onClick handler for the tabs
    var objNum, tobj;
    var doc = GM_config.frame.contentWindow.document;

    if ((typeof e) == 'number') {
        objNum = e;
    } else {
        objNum = /\_(\d+)\_/.exec(e.target.id)[1];
    }

    for (var i = 0; i < sectionTabs; i++) {
        tobj = doc.getElementById('section_' + i + '_tab');
        tdat = doc.getElementById('section_' + i);
...
GM_config.sections2tabs = function() {
    var doc = GM_config.frame.contentWindow.document;
    var $ = function(id) {
        return GM_config.frame.contentWindow.document.getElementById(id);
    };

    var rows = doc.querySelectorAll('.section_header');
    ...
    for (var i = 0; i < sectionTabs; i++) {
        rows[i].setAttribute('style', 'display:inline; padding-left:5px; padding-right:5px;');
        rows[i].id = 'section_' + i + '_tab';
        rows[i].addEventListener('click', GM_config.toggleSection, false);
        anch.appendChild(rows[i]);
        $('section_kids_' + i).style.marginLeft = "auto";
        $('section_kids_' + i).style.marginRight = "auto";
    }
    $('section_0').parentNode.insertBefore(anch, $('section_0'));
    $('section_0_tab').setAttribute('selected', true);
